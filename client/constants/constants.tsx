export const onboardingSwiperData: onboardingSwiperDataType[] = [
  {
    id: 1,
    title: "Initiez vous au code de manière ludique!",
    description: "Our engaging learning videos will spark",
    sortDescription: "your curiosity & enjoyable.",
    image: require("@/assets/onboarding/intro_1.png"),
  },
  {
    id: 2,
    title: "Découvrez une passion, augmentez vos compétences",
    description: "Our comprehensive our courses and expert",
    sortDescription: "instructors to guide every step.",
    image: require("@/assets/onboarding/intro_2.png"),
  },
  {
    id: 3,
    title: "Gagnez en expérience en effectuant des projets perso",
    description: "To earn an online certificate, individuals",
    sortDescription: "typically need to complete the required",
    sortDescription2: "coursework & assessments.",
    image: require("@/assets/onboarding/intro_3.png"),
  },
];

export const bannerData: BannerDataTypes[] = [
  {
    bannerImageUrl: require("@/assets/banner/1.jpg"),
  },
  {
    bannerImageUrl: require("@/assets/banner/2.webp"),
  },
  {
    bannerImageUrl: require("@/assets/banner/3.webp"),
  },
  {
    bannerImageUrl: require("@/assets/banner/4.webp"),
  },
];
